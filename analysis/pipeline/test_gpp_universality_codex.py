#!/usr/bin/env python3
"""
test_gpp_universality.py — GPP Universality Re-test (Codex-portable)
=====================================================================
...
Data inputs (place in /mnt/project/):
  - rar_points_unified.csv
  - galaxy_results_sparc_orig_haubner.csv
  - tng_mass_profiles.npz

Outputs (written to /mnt/user-data/outputs/gpp_universality/):
  - gpp_universality_3phase.png
  - summary_gpp_universality.json
...
# ============================================================
# Paths — Codex-portable
# ============================================================
PROJECT_DIR = '/mnt/project'
OUTPUT_DIR = '/mnt/user-data/outputs/gpp_universality'
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Data files — try /mnt/project/ first, fall back to relative paths
def find_file(name, subdirs=('', 'analysis/results', 'data')):
    """Locate a data file in multiple possible locations."""
    for sub in subdirs:
        path = os.path.join(PROJECT_DIR, sub, name)
    ...
    if os.path.isfile(local):
        return local
    # Fallback: ../results/
    up = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'results', name)
    if os.path.isfile(up):
        return up
    raise FileNotFoundError(f"Cannot find {name} in any expected location")


RAR_FILE = find_file('rar_points_unified.csv')
SPARC_META = find_file('galaxy_results_sparc_orig_haubner.csv')
TNG_FILE = find_file('tng_mass_profiles.npz')
...
def load_sparc_meta():
    ...
    for i in range(len(cols['galaxy'])):
        try:
            meta[cols['galaxy'][i]] = {
                'Q': int(cols['Q'][i]) if cols['Q'][i].strip() else 3,
                'Inc': float(cols['Inc'][i]) if cols['Inc'][i].strip() else 0,
                'fD': int(cols['fD'][i]) if cols['fD'][i].strip() else 0,
                'logMh': float(cols['logMh'][i]) if cols['logMh'][i].strip() else 11.0,
                'env_binary': cols['env_binary'][i] if 'env_binary' in cols else 'field',
                'Vflat': float(cols['Vflat'][i]) if cols['Vflat'][i].strip() else 0,
            }
...
def gpp_inversion_galaxy(r_kpc, g_dm, method='spline'):
    ...
    Given g_DM(r), computes:
      ...
      - ξ_proxy(r) = c_s² / |g_DM|                [healing length proxy]
    ...
    # Healing length proxy: ξ = c_s² / |g_DM|  [meters]
    xi_proxy = cs2 / np.abs(g_rho[valid])
...
def phase_a(pts, sparc_meta):
    ...
    # Apply quality cuts — HQ: Q=1, 45≤Inc≤75, fD∈{1,2}, N≥10
    hq_galaxies = []
    ...
    # Relaxed sample (Q≤2, 30<Inc<85) for comparison
    relaxed_galaxies = []
    for gname, idx_list in gal_groups.items():
        n_raw = len(idx_list)
        if n_raw < 8:
...
def phase_b(pts, sparc_meta):
    ...
    Model L: c_s² = a + b*logMh (linear mass-dependent, 3 parameters).
    """
    print("\n" + "=" * 70)
    ...
    # Model comparison: AIC/BIC on galaxy-level medians
    ...
    delta_AIC = AIC_U - AIC_L  # positive → Model L preferred
    w_U = np.exp(-0.5 * min(0, delta_AIC)) / (
        np.exp(-0.5 * min(0, delta_AIC)) + np.exp(-0.5 * max(0, -delta_AIC)))
...
def make_plots(phase_a_res, phase_b_res, phase_c_res):
    ...
    fig_path = os.path.join(OUTPUT_DIR, 'gpp_universality_3phase.png')
...
def main():
    ...
    print("\nLoading data...")
    print(f"  RAR file: {RAR_FILE}")
    print(f"  SPARC meta: {SPARC_META}")
    print(f"  TNG file: {TNG_FILE}")
    ...
    json_path = os.path.join(OUTPUT_DIR, 'summary_gpp_universality.json')
