#!/usr/bin/env python3
"""Single source-of-truth Branch D entrypoint (bec-dark-matter only)."""
...
BRANCHD_SCRIPT = EXPECTED_REPO / "analysis" / "pipeline" / "run_referee_required_tests.py"
...
def ensure_repo_guard(repo_root: Path, dataset_path: Path) -> None:
    ...
    required = [
        ...
        repo_root / "analysis" / "results" / "rar_points_unified.csv",
        dataset_path,
    ]
    missing = [str(p) for p in required if not p.exists()]
    if missing:
        raise RuntimeError("Missing required files for Branch D:\n- " + "\n- ".join(missing))
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run Branch D RAR-control refereeproof pipeline with strict repo guards.")
    ...
    p.add_argument(
        "--dry_run",
        action="store_true",
        help="Run guardrails + stamp only (no heavy science execution).",
    )
    return p
...
def main() -> None:
    ...
    out_dir = (
        ...
        if args.out_dir
        else (repo_root / "outputs" / "branchD_rar_control" / ts)
    )
    ...
    n_shuffles = 80 if args.smoke else int(args.n_shuffles)
    ...
    print("=== RUN STAMP (Branch D) ===", flush=True)
    ...
    stamp_path = out_dir / "run_stamp_branchD.json"
    stamp["status"] = "running"
    stamp_path.write_text(json.dumps(stamp, indent=2) + "\n", encoding="utf-8")

    if args.dry_run:
        stamp["status"] = "dry_run_ok"
        stamp_path.write_text(json.dumps(stamp, indent=2) + "\n", encoding="utf-8")
        print(f"[BRANCHD ENTRYPOINT] dry-run complete: {out_dir}")
        return
    ...
    stamp_path.write_text(json.dumps(stamp, indent=2) + "\n", encoding="utf-8")
    print(f"[BRANCHD ENTRYPOINT] done: {out_dir}")
